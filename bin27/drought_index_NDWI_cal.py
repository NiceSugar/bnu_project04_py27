...
def cal_VSWI_kernel(arr_NDVI,arr_LST):
    # NDVI/LST
    vswi = (arr_NDVI-arr_LST)/(arr_NDVI+arr_LST)
    return vswi
...
def loop_VSWI(NDVI,LST,save_folder,save_name,func):
    ...
    for npy in NDVI_npz:
        ...
        try:
            vals_NDVI = NDVI_npz[npy]
            vals_LST = LST_npz[npy]
        except:
            vals_NDVI = []
            vals_LST = []
...
def main():
    ...
    save_folder = this_root + 'monthly_data_npz_NDWI\\'

    ndvi_dir = data_root+'monthly_data_npz_b02\\'
    lst_dir = data_root+'monthly_data_npz_b06\\'
